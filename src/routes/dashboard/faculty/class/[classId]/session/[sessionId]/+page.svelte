<script lang="ts">
	...
	const sessionQuery = $derived(getSession(sessionId));
	const liveCount = $derived(getLiveAttendanceCount(sessionId));

	const classData = $derived(classQuery.current);
	let session = $state<ClassSession | null>(null);

	$effect(() => {
		const s = sessionQuery.current;
		if (s) session = s;
	});
		...
		<div class="flex-1 min-h-0 overflow-y-auto">
			...
			{:else}
				...
				>
					...
					{#if liveCount.current && liveCount.current.total > 0}
						<div class="mt-lg border-t border-hairline pt-base w-full">
